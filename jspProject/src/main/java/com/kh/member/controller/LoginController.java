...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 1) application : 여기에 담긴 데이트는 웹 에플리케이션 전역에서 다 꺼내서 쓸 수 있음
		 * 2) session : 여기에 담긴 데이터는 직접 지우기 전 까지, 세션이 만료(서버가 멈추거나, 브라우저가 종료)되기 전까지
		 * 				어떤 jsp든, 어떤 servelet이건 꺼내서 사용 할 수 있음.
		 * 3) request : 해당 영역에 담긴 데이터는 현재 이 request객체를 "포워딩 한 응답 jsp에서만" 꺼내 쓸 수 있음.
		 * 4) page : 해당 jsp담고 해당 jsp에서만 사용 할 수 있음.
		 * 
		...
		// 4) 처리된 결과를 가지고 사용자가 보게 될 응답 뷰룰 지정해서 포워딩 또는 url재요청
		if (loginUser == null) {
			// 조회결과가 없음 => 로그인 실패 => 에러문구가 보여지는 에러페이지 응답
			request.setAttribute("errorMsg", "로그인에 실패 하였습니다.");
...
//			view.forward(request, response);
			...
			//2. url 재요청 방식
			// 기존에 해당 페이지를 응답하는 url이 존재 한다면 사용 가능
			// 응답페이지 => index.js페이지 (jsp url 재요청)
